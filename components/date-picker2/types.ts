...
/**
 * 禁用时间配置
 * @en Disabled time configuration
 */
export interface DisabledTimeConfig {
    /**
     * 禁用小时函数
     * @en Disabled hours function
     * @param index - 小时索引 (0-23)
     */
    disabledHours?: (index?: number) => boolean | number[];

    /**
     * 禁用分钟函数
     * @en Disabled minutes function
     * @param index - 分钟索引 (0-59)
     */
    disabledMinutes?: (index?: number) => boolean | number[];

    /**
     * 禁用秒钟函数
     * @en Disabled seconds function
     * @param index - 秒钟索引 (0-59)
     */
...
export interface DatePanelProps {
     ...
     * @param date - Dayjs
     */
    disabledTime?: DisabledTimeConfig | ((date: Dayjs) => DisabledTimeConfig);
...
/**
 * 预设值类型，继承 Button 的属性但重新定义 value 和 label
 * @en Preset type, inherits Button properties but redefines value and label
 * - label: 预设按钮的显示文本 / Display text of the preset button
 * - value: 预设的日期值，可以是日期对象、日期数组或返回日期的函数 / Preset date value, can be a date object, date array, or a function that returns a date
 */
export interface PresetType extends Omit<ButtonProps, 'value' | 'label'> {
    /**
     * 预设按钮的显示文本
     * @en Display text of the preset button
     */
    label?: string;
    /**
     * 预设的日期值
     * @en Preset date value
     */
...
export interface RangePanelProps {
     ...
     */
    disabledTime?: DisabledTimeConfig | ((value: Dayjs[], inputType: number) => DisabledTimeConfig);
...
export interface DeprecatedProps {
    ...
    /**
     * 预设值（已废弃，请使用 preset）
     * @en Preset values (deprecated, use preset instead)
     * @deprecated use preset instead
     */
    ranges?: PresetType | PresetType[];
